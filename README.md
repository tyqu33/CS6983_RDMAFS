...
The printout should be like: Data dir: /mnt/d/CS6983/rdmafs/data Machine 0: initialized shared memory Machine 0: starting FUSE

4) Don't close the terminal from last step, open a new terminal and enter WSL:
`ls -la /tmp/mnt0`
The printout should be like: . and ..
Try `touch /tmp/mnt0/hello.txt`
Possible Errors:
a) The printout: touch: cannot touch '/tmp/mnt0/hello.txt': No space left on device
Stop FUSE (or Ctrl+C in the first terminal): `fusermount3 -u /tmp/mnt0`
Then clear up the old shared memory: `rm -f /dev/shm/rdmafs`
Clear up the old data: `rm -f /mnt/d/CS6983/rdmafs/data/*`
Restart: `cd /mnt/d/CS6983/rdmafs`
`./rdmafs -machine 0 -f /tmp/mnt0`

5) In terminal 2:
Write in: `echo "hello world" > /tmp/mnt0/hello.txt`
Check if the file gets created: `ls -la /mnt/d/CS6983/rdmafs/data/`
The printout should be like: 
.
..
m0.00001
Read: `cat /tmp/mnt0/hello.txt` See if writes and reads are functioning. The printout should be like: hello world

6) In terminal 2:
`./rdmafs -machine 1 -f /tmp/mnt1`
This starts machine 1.
The printout should be like: Data dir: /mnt/d/CS6983/rdmafs/data Machine 1: attached to existing shared memory Machine 1: starting FUSE
...
Terminal 3: Linux user. All operations here (touch, echo, cat, ls, etc.). It determines which machine should be operated on by the mount dir.
            Ops in /temp/mnt0/ -> calls to machine 0 on terminal 1.
            Ops in /temp/mnt1/ -> calls to machine 1 on terminal 2.
...
rm -f /dev/shm/rdmafs
```
Compile `gcc -Wall rdmafs.c -o rdmafs $(pkg-config --cflags --libs fuse3)`
then start mnt0 by `./rdmafs -machine 0 -f /tmp/mnt0`, start mnt1 by `./rdmafs -machine 1 -f /tmp/mnt1`, then run test script:
`bash test_rdmafs.sh`
